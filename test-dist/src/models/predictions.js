...
const getPredictions = async () => {
    try {
        ...
        const tableExists = await db_1.default.getQueryInterface().showAllTables();
...
const getPredictionsRaw = async () => {
    try {
        console.log('Exécution d\'une requête SQL brute pour débogage');
        const predictions = await db_1.default.query(`SELECT * FROM predictions_lstm`, { type: sequelize_1.QueryTypes.SELECT });
